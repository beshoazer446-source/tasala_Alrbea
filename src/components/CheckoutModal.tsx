...
export default function CheckoutModal({ onClose }: Props) {
  ...
  const [promoMsgType,  setPromoMsgType]  = useState<"ok"|"err">("ok");
  ...
  function weightLabel(wKey: string) {
    const map: Record<string,string> = {
      "125g":"125 جرام","250g":"250 جرام","500g":"500 جرام",
      "750g":"750 جرام","1kg":"1000 جرام","unit":"قطعة"
    };
  ...
  async function applyPromo() {
    ...
    const res  = await fetch('/api/promo', {
      ...
      body: JSON.stringify({ code: promoCode.trim(), total: cartTotal }),
    });
    ...
    if (data.success) {
      ...
      setPromoMsg(`✅ تم تطبيق خصم ${data.discount} ج.م!`);
      setPromoMsgType("ok");
    } else {
      setPromoDiscount(0);
      setPromoApplied("");
      const msgs: Record<string,string> = {
        ...
        max_uses:  "❌ تم استنفاد هذا الكود",
        wrong_day: `❌ الكود يعمل ${data.valid_days} بس`,
        min_order: `❌ الحد الأدنى للطلب ${data.min} ج.م`,
      };
      setPromoMsg(msgs[data.error] ?? "❌ خطأ في تطبيق الكود");
      setPromoMsgType("err");
    }
  }
  ...
  function buildMessage() {
    ...
    const lines = cart.map((item, idx) => {
      ...
      return `${idx + 1}. *${itemName}* (${item.qty} × ${weightLabel(item.selectedWeight)}) — ${item.finalPrice * item.qty} ج.م`;
    });
    const promoLine = promoApplied
      ? `\n*كود الخصم:* ${promoApplied} (خصم ${promoDiscount} ج.م)` : "";

    return `🛒 *طلب جديد من تسالي الربيع 🌿*
*الفرع:* الغردقة
*الاسم:* ${name.trim()}
*الهاتف:* ${phone.trim()}
*العنوان:* ${address.trim()}
*طريقة الدفع:* ${payLabels[payMethod]}${promoLine}
----------------------------
${lines.join("\n")}
----------------------------
*إجمالي المنتجات:* ${cartTotal} ج.م${promoDiscount > 0
  ? `\n*الخصم:* - ${promoDiscount} ج.م\n*الإجمالي بعد الخصم:* ${finalTotal} ج.م`
...
يرجى التواصل لترتيب التوصيل 🙏`;
  ...
  const payOptions: { key: PayMethod; icon: string; label: string; sub: string }[] = [
    { key:"whatsapp_cod", icon:"🚚", label:"الدفع عند التوصيل", sub:"ادفع لما البضاعة توصلك" },
      ...
      <div className={styles.modal}>
        <div className={styles.modalHeader}>
          <div className={styles.headerLeft}>
            ...
            <div>
              <h2 className={styles.modalTitle}>إتمام الطلب عبر واتساب</h2>
              <p className={styles.modalSub}>أدخل بياناتك وسنرسل طلبك مباشرة</p>
            </div>
          </div>
        ...
        <div className={styles.modalBody}>
          ...
          <div className={styles.orderSummary}>
            <p className={styles.summaryTitle}>ملخص الطلب</p>
            {cart.map(item => (
            ...
            {promoDiscount > 0 && (
              <>
                <div className={styles.summaryDiscount}>
                  <span>🎁 خصم كود &ldquo;{promoApplied}&rdquo;</span>
                  <span className={styles.discountNum}>- {promoDiscount} {t.egp}</span>
                </div>
                <div className={`${styles.summaryTotal} ${styles.finalTotal}`}>
                  <span>الإجمالي بعد الخصم</span>
                  <span className={styles.finalNum}>{finalTotal} {t.egp}</span>
                </div>
              </>
            )}
          </div>
          ...
          <div className={styles.form}>
            <div className={styles.fieldRow}>
              ...
              <div className={styles.field}>
                ...
                <input className={`${styles.input} ${errors.phone ? styles.inputError : ""}`}
                  placeholder="01xxxxxxxxx" dir="ltr" type="tel"
            ...
            <div className={styles.promoSection}>
              <label className={styles.label}>🎁 كود الخصم (اختياري)</label>
              {promoApplied ? (
                <div className={styles.promoApplied}>
                  <span>✅ كود <strong>{promoApplied}</strong> — خصم {promoDiscount} ج.م</span>
                  <button className={styles.promoRemove} onClick={removePromo}>✕ إزالة</button>
                </div>
              ) : (
                <div className={styles.promoRow}>
                  <input
                    className={styles.promoInput}
                    placeholder="TASALI50"
                    value={promoCode}
                    ...
                    dir="ltr"
                  />
                  <button className={styles.promoBtn} onClick={applyPromo}
              ...
              {promoMsg && !promoApplied && (
                <p className={`${styles.promoMsg} ${promoMsgType==="ok" ? styles.promoOk : styles.promoErr}`}>
                  {promoMsg}
                </p>
              )}
            </div>
